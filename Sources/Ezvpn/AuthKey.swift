...
enum AuthKey {
    ...
    /// Generate a fresh ed25519 keypair. Nil only if the FFI misbehaves (or the
    /// system RNG is unavailable).
    static func generate() -> Keypair? {
        // The FFI writes a truncated prefix of the document — secret material
        // included — when the buffer is too small, so zero it before it goes
        // out of scope rather than leaving a partial secret in memory.
        var buf = [CChar](repeating: 0, count: 1024)
        defer { for index in buf.indices { buf[index] = 0 } }
        guard ezvpn_generate_client_key(&buf, buf.count) == 1,
              let data = String(cString: buf).data(using: .utf8),
              let object = try? JSONSerialization.jsonObject(with: data) as? [String: Any],
              let secretKey = object["secret_key"] as? String,
    ...
    static func publicKey(forSecret secret: String) -> String? {
        ...
        let ok = secret.withCString { ezvpn_client_public_key($0, &buf, buf.count) }
